...
<nav x-data="{ open: false }" class="navbar-dashboard sticky top-0 z-50">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('company-rules.index') }}" class="text-white font-bold text-xl">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('company-rules.create')" :active="request()->routeIs('company-rules.create')">
                        {{ __('Create New') }}
                    </x-nav-link>
                    ...
                    @if (Auth::check() && Auth::user()->grade >= 8)
                        ...
                        <x-nav-link :href="route('approvals.index')" :active="request()->routeIs('approvals.index')">
                            <span class="relative">
                                {{ __('Approve Document') }}
                                @if ($pendingCount > 0)
                                    <span class="absolute top-[-0.75rem] right-[-1.5rem] inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-blue-600 rounded-full">
                                        {{ $pendingCount }}
                                    </span>
                                @endif
                            </span>
                        ...
                        <x-slot name="trigger">
                            <button class="relative inline-flex items-center p-2 text-sm font-medium text-center text-white hover:text-white/80 focus:outline-none">
                                ...
                                @if($unreadNotificationsCount > 0)
                                    <div class="absolute inline-flex items-center justify-center w-7 h-7 text-sm font-bold text-white bg-yellow-500 border-2 border-white rounded-full -top-2 -end-2 shadow-lg">{{ $unreadNotificationsCount }}</div>
                                @endif
                            </button>
                        </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('company-rules.create')" :active="request()->routeIs('company-rules.create')">
                {{ __('Create New') }}
            </x-responsive-nav-link>
            ...
            @if (Auth::check() && Auth::user()->grade >= 8)
                ...
                <x-responsive-nav-link :href="route('approvals.index')" :active="request()->routeIs('approvals.index')">
                    <span class="relative">
                        {{ __('Approve Document') }}
                        @if ($pendingCount > 0)
                            <span class="absolute top-[-0.25rem] right-[-1.5rem] inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">
                                {{ $pendingCount }}
                            </span>
                        @endif
                    </span>
