...
class CompanyRuleController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($status === 'approved') {
            $rulesQuery->where('status', 'Approved')->where('is_obsolete', false);
        } elseif ($status === 'obsolete') {
            $rulesQuery->where('is_obsolete', true);
        } elseif ($status === 'pending_send_back') {
    ...
    {
        $revisableRules = CompanyRule::where('creator_id', Auth::user()->id)
    ...
    public function storeRevision(Request $request, CompanyRule $rule)
    {
        ...
        $newRule->creator_id = Auth::user()->id;
